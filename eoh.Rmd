...
```{r}
#Importo datos
pernoctes_residencia <- read_csv("/srv/DataDNMYE/eoh/Pernoctes-residentes-y-no-residentes.csv")
viajeros_residencia <- read_csv("/srv/DataDNMYE/eoh/Viajeros-hospedados-residentes-y-no-residentes.csv")
...
datos_grafico1 <- eoh %>%
  ...
  mutate(pernoctes = round(pernoctes/1000, 3)) %>% 
   mutate(pernoctes = case_when(
...
grafico1 <- datos_grafico1 %>% 
  ...
  scale_x_date(date_breaks = "1 months",
               date_labels = "%b %y",
               expand = c(0,10)) +
  scale_y_continuous(breaks = seq(min(datos_grafico1$pernoctes, na.rm = T), max(datos_grafico1$pernoctes, na.rm = T), by = 500),
  ...
  labs(
       title = tolower(glue("en miles de noches. enero 2018-{mes} {anio}. <span style='color:{cols_arg2[6]}'>residentes</span> y <span style='color:{cols_arg2[1]}'>no residentes</span>")),subtitle="",
                       x="",y="", fill = "",
...
tabla1_A <- tabla1 %>% 
  ...
  rbind(totales) %>% 
  mutate(pernoctes  = round(pernoctes / 1000, 3),
         viajeros  = round(viajeros / 1000, 3),
         estadia = round(pernoctes / viajeros, 3)) %>%
  mutate(mes = month(indice_tiempo),
         anio = year(indice_tiempo)) %>% 
  group_by(origen_viajeros, mes) %>% 
  mutate(var_ia_pernoctes  = round(pernoctes / lag(pernoctes, n = 1) -1, 3),
         var_ia_viajeros  = round(viajeros / lag(viajeros, n = 1) -1, 3),
         var_ia_estadia  = round(estadia / lag(estadia, n = 1) -1, 3)) %>%
  ungroup() %>% 
  ...
  mutate(var_ia_acum_pernoctes = round(acum_pernoctes/lag(acum_pernoctes, n = 1) -1, 3),
         var_ia_acum_viajeros = round(acum_viajeros/lag(acum_viajeros, n = 1) -1, 3),
         var_ia_acum_estadia = round(acum_estadia/lag(acum_estadia, n = 1) -1, 3)) %>%
  ungroup() %>% 
...
acumulados <- tabla1_A %>% select(1,2, 11:16) %>%
  mutate(indice_tiempo = "Acum. 2021") %>% 
...
#Importo datos
pernoctaciones_categoria <- read_csv("/srv/DataDNMYE/eoh/pernoctaciones-por-categoria.csv")
plazas_categoria <- read_csv("/srv/DataDNMYE/eoh/plazas-disponibles-por-categoria.csv")

tabla2 <- pernoctaciones_categoria %>% left_join(plazas_categoria) %>% 
  mutate(indice_tiempo = ym(indice_tiempo)) %>% 
  filter(month(indice_tiempo) == month(fecha) & year(indice_tiempo) >= year(fecha)-1) %>%
  mutate(pernoctes = round(pernoctes/1000, 3),
         plazas_disponibles = round(plazas_disponibles/1000, 3),
  ...
  mutate(pernoctes = case_when(pernoctes == 0 ~ `is.na<-`(pernoctes),
      TRUE ~ pernoctes)) %>% 
  mutate(var_ia_pernoctes  = round(pernoctes / lag(pernoctes, n = 1) -1, 3)) %>%
  ungroup() %>%
  pivot_wider(names_from = "origen_viajeros", values_from = c("pernoctes", "var_ia_pernoctes")) %>% 
  filter(year(indice_tiempo) == year(fecha)) %>%
  arrange(desc(categoria_del_hotel)) %>% #AD HOC JULIO 2021
  ...
  # mutate(pernoctes_nores = case_when(categoria_del_hotel %in% 
  #                                      c("1 y 2 estrellas", "Para Hoteles") ~
  #                                      `is.na<-`(pernoctes_nores),
  #                                    TRUE ~ pernoctes_nores))
...
+ En `r tolower(mes)` de `r anio`, se hospedaron `r format(round(as.numeric(tablafinal_ficha[3,5]),1), decimal.mark = ",", big.mark = ".")` viajeros (creciendo `r format(round(as.numeric(tablafinal_ficha[3,6]*100),1), decimal.mark = ",", big.mark = ".")`% en comparación con el mismo mes de 2020) con una estadía de `r format(round(as.numeric(tablafinal_ficha[3,7]),1), decimal.mark = ",", big.mark = ".")` noches en promedio (`r format(round(as.numeric(tablafinal_ficha[3,8]*100),1), decimal.mark = ",", big.mark = ".")`% i.a). **El total de pernoctes registrados ascendió a `r format(round(as.numeric(tablafinal_ficha[3,3]),1), decimal.mark = ",", big.mark = ".")` mil noches (`r format(round(as.numeric(tablafinal_ficha[3,4]*100),0), decimal.mark = ",", big.mark = ".")`% i.a)**. 

+ En lo que llevamos de 2021, las pernoctaciones presentan una caída de 11% i.a., en tanto que los viajeros lo hacen en un 7%; sin embargo, las pernoctaciones y viajeros residentes en el país crecieron por primera vez en el acumulado anual (8% y 19%, respectivamente).
...
+ **Los establecimientos de `r tabla2[tabla2$pernoctes_total == sort(tabla2$pernoctes_total)[length(tabla2$pernoctes_total)-1], "categoria_del_hotel"]` concentraron la mayor cantidad de pernoctaciones (`r format(round(as.numeric(tabla2[tabla2$pernoctes_total == sort(tabla2$pernoctes_total)[length(tabla2$pernoctes_total)-1], "pernoctes_total"]),digits=1),decimal.mark = ",", big.mark = ".")` mil)**, con una tasa de ocupación de plazas de `r format(round(as.numeric(tabla2[tabla2$pernoctes_total == sort(tabla2$pernoctes_total)[length(tabla2$pernoctes_total)-1], "pernoctes_total"])/as.numeric(tabla2[tabla2$pernoctes_total == sort(tabla2$pernoctes_total)[length(tabla2$pernoctes_total)-1], "plazas_disponibles"])*100,digits=1),decimal.mark = ",", big.mark = ".")`%; seguidos por los establecimientos `r tabla2[tabla2$pernoctes_total == sort(tabla2$pernoctes_total)[length(tabla2$pernoctes_total)-2], "categoria_del_hotel"]`, con `r format(round(as.numeric(tabla2[tabla2$pernoctes_total == sort(tabla2$pernoctes_total)[length(tabla2$pernoctes_total)-2], "pernoctes_total"]),digits=0),decimal.mark = ",", big.mark = ".")` mil pernoctes y una ocupación de `r format(round(as.numeric(tabla2[tabla2$pernoctes_total == sort(tabla2$pernoctes_total)[length(tabla2$pernoctes_total)-2], "pernoctes_total"])/as.numeric(tabla2[tabla2$pernoctes_total == sort(tabla2$pernoctes_total)[length(tabla2$pernoctes_total)-2], "plazas_disponibles"])*100,digits=1),decimal.mark = ",", big.mark = ".")`% de las plazas disponibles.
...
tabla1_ficha <- tablafinal_ficha %>%
  ...
  cols_label(
    ...
    pernoctes = md("Miles de noches"),
    var_ia_pernoctes = md("Var % i.a."),
    viajeros  = md("Miles de viajeros") ,
...
tabla2_ficha <- tabla2 %>% 
  ...
  cols_label(
    ...
    plazas_disponibles = md("Miles"),
    pernoctes_total = md("Miles de noches"),
    var_ia_pernoctes_total  = md("Var % i.a."),
    pernoctes_res = md("Miles de noches"),
    var_ia_pernoctes_res  = md("Var % i.a."),
    pernoctes_nores = md("Miles de noches"),
...
grafico2 <- datos_grafico2 %>% 
  ...
  geom_text(aes(categoria_del_hotel, cantidad,
                label = format(cantidad, big.mark = ".", decimal.mark = ",", digits = 0),
  ...
  theme(
    plot.title = element_markdown(size = 14, family = familia_fuente, colour = "#AAAAAA", hjust = 2.5),
    plot.subtitle  = element_markdown(family = familia_fuente, size = 12, hjust = 2),
    plot.caption = element_markdown(size = 8, family = familia_fuente),
...
## TABLA 3
#Importo datos
pernoctes_destino <- read.csv("/srv/DataDNMYE/eoh/Pernoctes-residentes-y-no-residentes-por-destino.csv", encoding = "UTF-8")
viajeros_destino <- read.csv("/srv/DataDNMYE/eoh/Viajeros-hospedados-residentes-y-no-residentes-por-destino.csv", encoding = "UTF-8")
...
tabla3 <- destinos %>% select(-observaciones) %>%
  ...
  mutate(pernoctes_total = pernoctes_residentes + pernoctes_no_residentes,
         ...
         estadia_total = round(pernoctes_total/viajeros_total, 1)) %>%  #AD HOC JULIO 2021
  mutate(pernoctes_no_residentes = case_when(region_de_destino == "Cuyo"  ~
                                       `is.na<-`(pernoctes_no_residentes),
                                     TRUE ~ pernoctes_no_residentes),
         viajeros_no_residentes = case_when(region_de_destino == "Cuyo"  ~
                                       `is.na<-`(viajeros_no_residentes),
                                     TRUE ~ viajeros_no_residentes),
         estadia_no_residentes = case_when(region_de_destino == "Cuyo"  ~
                                       `is.na<-`(estadia_no_residentes),
...
grafico3 <- datos_grafico3 %>% ggplot() +
  ...
  labs(#title = "**Pernoctaciones por región**",
       subtitle = tolower(glue("**{mes} {anio}.** <span style='color:{cols_arg2[6]}'>**residentes**</span> **y** <span style='color:{cols_arg2[2]}'>**no residentes**</span>")),
                       x="",y="", fill = "",
       caption = "Nota: No se presentan los pernoctes de no residentes para la región Córdoba por la naturaleza de las cosas.   \n\nFuente: Encuesta de Ocupación Hotelera (EOH), INDEC y Dirección Nacional de Mercados y Estadística, Subsecretaría de Desarrollo Estratégico, Ministerio de Turismo y Deportes")
...
+ **La mayor cantidad de pernoctaciones se registraron en la región `r as.character(tabla3[tabla3$pernoctes_total == sort(tabla3$pernoctes_total, na.last = T)[length(tabla3$pernoctes_total)-7], "region_de_destino"] %>% filter(!is.na(region_de_destino)))`**, con `r format(round(as.numeric(max(tabla3$pernoctes_total, na.rm = T)), 1), decimal.mark = ",", big.mark = ".")` mil noches, seguida por `r as.character(tabla3[tabla3$pernoctes_total == sort(tabla3$pernoctes_total, na.last = T)[length(tabla3$pernoctes_total)-8], "region_de_destino"] %>% filter(!is.na(region_de_destino)))` (`r format(round(as.numeric(tabla3[tabla3$pernoctes_total == sort(tabla3$pernoctes_total, na.last = T)[length(tabla3$pernoctes_total)-8], "pernoctes_total"] %>% filter(!is.na(pernoctes_total))), 1), decimal.mark = ",", big.mark = ".")` mil) y `r as.character(tabla3[tabla3$pernoctes_total == sort(tabla3$pernoctes_total, na.last = T)[length(tabla3$pernoctes_total)-9], "region_de_destino"] %>% filter(!is.na(region_de_destino)))` (`r format(round(as.numeric(tabla3[tabla3$pernoctes_total == sort(tabla3$pernoctes_total, na.last = T)[length(tabla3$pernoctes_total)-9], "pernoctes_total"] %>% filter(!is.na(pernoctes_total))), 1), decimal.mark = ",", big.mark = ".")` mil noches).

+ **En lo que respecta a la distribución de los viajeros, es la región `r as.character(tabla3[tabla3$viajeros_total == sort(tabla3$viajeros_total, na.last = T)[length(tabla3$viajeros_total)-7], "region_de_destino"] %>% filter(!is.na(region_de_destino)))` la que concentró la mayor cantidad (`r format(round(as.numeric(tabla3[tabla3$viajeros_total == sort(tabla3$viajeros_total, na.last = T)[length(tabla3$viajeros_total)-7], "viajeros_total"] %>% filter(!is.na(viajeros_total))), 1), decimal.mark = ",", big.mark = ".")` mil)**, seguida por `r as.character(tabla3[tabla3$viajeros_total == sort(tabla3$viajeros_total, na.last = T)[length(tabla3$viajeros_total)-8], "region_de_destino"] %>% filter(!is.na(region_de_destino)))` y `r as.character(tabla3[tabla3$viajeros_total == sort(tabla3$viajeros_total, na.last = T)[length(tabla3$viajeros_total)-9], "region_de_destino"] %>% filter(!is.na(region_de_destino)))` (con `r format(round(as.numeric(tabla3[tabla3$viajeros_total == sort(tabla3$viajeros_total, na.last = T)[length(tabla3$viajeros_total)-8], "viajeros_total"] %>% filter(!is.na(viajeros_total))), 1), decimal.mark = ",", big.mark = ".")` mil y `r format(round(as.numeric(tabla3[tabla3$viajeros_total == sort(tabla3$viajeros_total, na.last = T)[length(tabla3$viajeros_total)-9], "viajeros_total"] %>% filter(!is.na(viajeros_total))), 1), decimal.mark = ",", big.mark = ".")` mil viajeros hospedados, respectivamente).

+ Las mayores estadías en `r tolower(mes)` de `r anio` se estimaron en `r as.character(tabla3[tabla3$estadia_total == sort(tabla3$estadia_total, na.last = T)[length(tabla3$estadia_total)-7], "region_de_destino"] %>% filter(!is.na(region_de_destino)))` con `r format(round(as.numeric(tabla3[tabla3$estadia_total == sort(tabla3$estadia_total, na.last = T)[length(tabla3$estadia_total)-7], "estadia_total"] %>% filter(!is.na(estadia_total))), 1), decimal.mark = ",", big.mark = ".")` noches promedio.
...
```{r}
#Importo datos
pernoctaciones_localidad <-read.csv("/srv/DataDNMYE/eoh/pernoctes-viajeros-y-estadia-media-por-localidad.csv", encoding = "UTF-8")

#Localidades que no corresponde mostrar datos (AD HOC,INDEC)
localidades_insuficiencia <- c("Formosa", "Posadas", "Puerto Iguazú")
localidades_sin_datos <- c("Gualeguaychú", "Mar del Plata", "CABA", "Catamarca", "Santiago del Estero", "La Angostura", "San Martín de los Andes", "Rio Gallegos", "Viedma", "Santa Rosa")
...
tabla4 <- pernoctaciones_localidad %>% 
  ...
  select(region_de_destino, localidad_de_destino, pernoctes, var_ia_pernoctes,
         viajeros, var_ia_viajeros,
         estadia_media_en_noches, var_ia_estadia) %>% #AD HOC JULIO 2021
...
tabla4_B_ficha <- tabla4_B %>% arrange(region_de_destino) %>%
  ...
  tab_source_note(
    source_note = md(
      "/// : Dato que no corresponde presentar debido a la naturaleza de las cosas. En julio 2020, la actividad en muchos de los establecimientos hoteleros y parahoteleros fue considerada nula debido a la emergencia sanitaria; por ello la comparación i.a. de julio 2021 no es posible realizarla.")
  ) %>% 
  tab_source_note(
    source_note = md(
      "**Fuente**: Encuesta de Ocupación Hotelera (EOH) INDEC y Dirección Nacional de Mercados y Estadística, Subsecretaría de Desarrollo Estratégico, Ministerio de Turismo y Deportes")
