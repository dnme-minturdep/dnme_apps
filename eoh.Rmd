...
#`r tabla2[tabla2$pernoctes_total == sort(tabla2$pernoctes_total)[length(tabla2$pernoctes_total)-1], "categoria_del_hotel"]`
```
...
+ **La mayor cantidad de pernoctaciones se registraron en la región `r as.character(arrange(filter(tabla3, indice_tiempo == fecha), desc(pernoctes_total))[1,"region_de_destino"])`**, con `r format(round(as.numeric(arrange(filter(tabla3, indice_tiempo == fecha), desc(pernoctes_total))[1,"pernoctes_total"]), 1), decimal.mark = ",", big.mark = ".")` mil noches, seguida por `r as.character(arrange(filter(tabla3, indice_tiempo == fecha), desc(pernoctes_total))[2,"region_de_destino"])` (`r format(round(as.numeric(arrange(filter(tabla3, indice_tiempo == fecha), desc(pernoctes_total))[2,"pernoctes_total"]), 1), decimal.mark = ",", big.mark = ".")` mil) y `r as.character(arrange(filter(tabla3, indice_tiempo == fecha), desc(pernoctes_total))[3,"region_de_destino"])` (`r format(round(as.numeric(arrange(filter(tabla3, indice_tiempo == fecha), desc(pernoctes_total))[3,"pernoctes_total"]), 1), decimal.mark = ",", big.mark = ".")` mil noches).

+ **En lo que respecta a la distribución de los viajeros, es la región `r as.character(arrange(filter(tabla3, indice_tiempo == fecha), desc(viajeros_total))[1,"region_de_destino"])` la que concentró la mayor cantidad (`r format(round(as.numeric(arrange(filter(tabla3, indice_tiempo == fecha), desc(viajeros_total))[1,"viajeros_total"]), 1), decimal.mark = ",", big.mark = ".")` mil)**, seguida por `r as.character(arrange(filter(tabla3, indice_tiempo == fecha), desc(viajeros_total))[2,"region_de_destino"])` y `r as.character(arrange(filter(tabla3, indice_tiempo == fecha), desc(viajeros_total))[3,"region_de_destino"])` (con `r format(round(as.numeric(arrange(filter(tabla3, indice_tiempo == fecha), desc(viajeros_total))[2,"viajeros_total"]), 1), decimal.mark = ",", big.mark = ".")` mil y `r format(round(as.numeric(arrange(filter(tabla3, indice_tiempo == fecha), desc(viajeros_total))[3,"viajeros_total"]), 1), decimal.mark = ",", big.mark = ".")` mil viajeros hospedados, respectivamente).

+ Las mayores estadías en `r tolower(mes)` de `r anio` se estimaron en `r as.character(arrange(filter(tabla3, indice_tiempo == fecha), desc(estadia_total))[1,"region_de_destino"])` con `r format(round(as.numeric(arrange(filter(tabla3, indice_tiempo == fecha), desc(estadia_total))[1,"estadia_total"]), 1), decimal.mark = ",", big.mark = ".")` noches promedio.
...
localidades_sin_datos <- c("Puerto Iguazú", "Resistencia", "Santa Fe")
localidades_insuficiencia <- c("Puerto Iguazú", "Rafaela", "Posadas")
...
tabla4 <- pernoctaciones_localidad %>% 
  ...
  mutate(across(3:8, ~ case_when(localidad_de_destino %in% localidades_insuficiencia ~
                                   `is.na<-`(.),
                                 TRUE ~ .))) %>% 
  mutate(var_ia_pernoctes = case_when(localidad_de_destino == "Formosa" ~ 6.74,
                                      TRUE ~ var_ia_pernoctes),
         var_ia_viajeros = case_when(localidad_de_destino == "Formosa" ~ 8.14,
                                      TRUE ~ var_ia_viajeros),
         var_ia_estadia = case_when(localidad_de_destino == "Formosa" ~ -0.15,
                                      TRUE ~ var_ia_estadia))
```
