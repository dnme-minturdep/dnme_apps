...
- En junio de 2021, se hospedaron 288,9 mil viajeros (creciendo 1.100% en comparación con el mismo mes de 2020) con una estadía de 2,3 noches en promedio (-27% i.a). **El total de pernoctes registrados ascendió a 653,9 mil noches (781% i.a)**. 

- El 98% de los viajeros hospedados y las pernoctaciones registradas fueron de residentes.

- **Los establecimientos de 3 estrellas, Boutiques y Aparts concentraron la mayor cantidad de pernoctaciones (224,5 mil)**, con una tasa de ocupación de plazas de 11,4%; seguidos por los hoteles de 4 y 5 estrellas, con una ocupación de 11,7% de las plazas disponibles.
...
grafico1 <- datos_grafico1 %>% 
  ...
  theme(
    ...
    plot.subtitle  = element_markdown(family = familia_fuente, size = 12),
  ...
  labs(title = glue("**Evolución de las pernoctaciones según origen**"),
       subtitle = tolower(glue("**en miles de noches. enero 2018-{mes} {anio}.** <span style='color:{cols_arg2[6]}'>**residentes**</span> **y** <span style='color:{cols_arg2[1]}'>**no residentes**</span>")),
                       x="",y="", fill = "",
...
tabla2 <- pernoctaciones_categoria %>% left_join(plazas_categoria) %>% 
  ...
  filter(year(indice_tiempo) == year(fecha)) %>%
  arrange(desc(categoria_del_hotel)) %>% #AD HOC JUNIO 2021
...
tabla3 <- destinos %>% select(-observaciones) %>%
  ...
  mutate(pernoctes_total = pernoctes_residentes + pernoctes_no_residentes,
         ...
         estadia_total = round(pernoctes_total/viajeros_total, 1)) %>%  #AD HOC JUNIO 2021
  mutate(pernoctes_no_residentes = case_when(region_de_destino == "Córdoba"  ~
                                       `is.na<-`(pernoctes_no_residentes),
                                     TRUE ~ pernoctes_no_residentes),
         viajeros_no_residentes = case_when(region_de_destino == "Córdoba"  ~
                                       `is.na<-`(viajeros_no_residentes),
                                     TRUE ~ viajeros_no_residentes),
         estadia_no_residentes = case_when(region_de_destino == "Córdoba"  ~
...
+ **La mayor cantidad de pernoctaciones se registraron en la región Patagonia**, con 144,5 mil noches, seguida por CABA (115,6 mil) y Norte (115,5 mil noches).

+ **En lo que respecta a la distribución de los viajeros, es la región Norte la que concentró la mayor cantidad (57,3 mil)**, seguida por Patagonia y CABA (con 57 mil y 49,9 viajeros hospedados, respectivamente).

+ Las mayores estadías en junio de 2021 se estimaron en Cuyo y Patagonia con 2,5 noches promedio, en ambos casos.
...
localidades <- c("Mar del Plata", "La Rioja", "San Luis", "Gualeguaychú", "Posadas", "Rafaela", "Catamarca", "Santiago del Estero", "Rio Gallegos", "San Martín de los Andes", "Ushuaia")
...
tabla4 <- pernoctaciones_localidad %>% 
  ...
  select(region_de_destino, localidad_de_destino, pernoctes, var_ia_pernoctes,
         viajeros, var_ia_viajeros,
         estadia_media_en_noches, var_ia_estadia) %>% 
  mutate(var_ia_pernoctes = case_when(localidad_de_destino %in% localidades ~
                                       `is.na<-`(var_ia_pernoctes),
                                     TRUE ~ var_ia_pernoctes),
         var_ia_viajeros = case_when(localidad_de_destino %in% localidades ~
                                       `is.na<-`(var_ia_viajeros),
                                     TRUE ~ var_ia_viajeros),
         var_ia_estadia = case_when(localidad_de_destino %in% localidades ~
                                       `is.na<-`(var_ia_estadia),
                                     TRUE ~ var_ia_estadia))
  # mutate(var_ia_acum_pernoctes = case_when(
  # localidad_de_destino %in% c("Formosa", "Las Grutas", "Resistencia", "Puerto Iguazú", "Villa Gesell") ~ NaN,
  # TRUE ~ var_ia_acum_pernoctes),
  # var_ia_acum_viajeros = case_when(
  # localidad_de_destino %in% c("Formosa", "Las Grutas", "Resistencia", "Puerto Iguazú", "Villa Gesell") ~ NaN,
  # TRUE ~ var_ia_acum_viajeros),
  # var_ia_acum_estadia = case_when(
  # localidad_de_destino %in% c("Formosa", "Las Grutas", "Resistencia", "Puerto Iguazú", "Villa Gesell") ~ NaN,
  # TRUE ~ var_ia_acum_estadia)) %>%
  # mutate(
  #   pernoctes = case_when(localidad_de_destino %in% c("Formosa", "Resistencia",
  #                                          "Puerto Iguazú", "Villa Gesell") ~ NaN,
  #   TRUE ~ as.numeric(pernoctes)),
  #   viajeros = case_when(localidad_de_destino %in% c("Formosa", "Resistencia",
  #                                          "Puerto Iguazú", "Villa Gesell") ~ NaN,
  #   TRUE ~ as.numeric(viajeros)),
  #   estadia_media_en_noches = case_when(localidad_de_destino %in% c("Formosa",
  #                                                                      "Resistencia",
  #                                                                      "Puerto Iguazú",
  #                                                                      "Villa Gesell") ~ NaN,
  #   TRUE ~ estadia_media_en_noches)) %>%
  #   filter(!localidad_de_destino == "Caleta Olivia")
...
tabla4_A_ficha <- tabla4_A %>% arrange(region_de_destino) %>%
  ...
  fmt_missing(
    columns = 3:8,
      rows = localidad_de_destino %in% c("Formosa", "Mar Chiquita"),
      missing_text = "..."
    ) %>%
  tab_options(table.align = "left",
              table.width = 800)
...
tabla4_B_ficha <- tabla4_B %>% arrange(region_de_destino) %>%
  ...
  tab_source_note(
    source_note = md(
      "/// : Dato que no corresponde presentar debido a la naturaleza de las cosas. En junio 2020, la actividad en muchos de los establecimientos hoteleros y parahoteleros fue considerada nula debido a la emergencia sanitaria; por ello la comparación i.a. de junio 2021 no es posible realizarla.")
  ) %>% 
  tab_source_note(
    source_note = md(
      "**Fuente**: Encuesta de Ocupación Hotelera (EOH) INDEC y Dirección Nacional de Mercados y Estadística, Subsecretaría de Desarrollo Estratégico, Ministerio de Turismo y Deportes")
  ...
  fmt_missing(
    columns = 3:8,
      rows = localidad_de_destino %in% c("Formosa", "Mar Chiquita", "Neuquén"),
      missing_text = "..."
    ) %>%
  tab_options(table.align = "left",
              table.width = 800)
...
tabla4_A_ficha
tabla4_B_ficha
#gtsave(tabla4_A_ficha, "salidas/tabla4A.png")
#gtsave(tabla4_B_ficha, "salidas/tabla4B.png")
#htmltools::save_html(html = tabla4_A_ficha, file = "salidas/tabla4A.html")

#webshot::webshot("salidas/tabla4A.html", file="salidas/tabla4A.png", delay=3, vwidth = 900, vheight = 300)

#htmltools::save_html(html = tabla4_B_ficha, file = "salidas/tabla4B.html")

#webshot::webshot("salidas/tabla4B.html", file="salidas/tabla4B.png", delay=3, vwidth = 900, vheight = 300)
...
```{r fig.height=5, fig.width=10}
### TABLA 4 - resumen establecimientos del mes por región

#Importo datos esablecimientos
establecimientos_covid <- read.csv("/srv/DataDNMYE/eoh/establecimientos-abiertos-covid-por-region.csv", encoding = "UTF-8")

tabla5 <- establecimientos_covid %>%
  mutate(indice_de_tiempo = ym(indice_de_tiempo)) %>% 
  filter(ymd(indice_de_tiempo) == ymd(fecha)) %>% 
  mutate(participacion = round(establecimientos_abiertos/sum(establecimientos_abiertos), 3)) %>% 
  select(region_de_destino, establecimientos_abiertos, participacion)

#Agrego fila total
total <- c("Total", sum(tabla5$establecimientos_abiertos), 1)

tabla5 <- rbind(tabla5, total) %>% 
  mutate(establecimientos_abiertos = as.numeric(establecimientos_abiertos),
         participacion = as.numeric( participacion))


tabla5_ficha <- tabla5 %>% arrange(establecimientos_abiertos) %>% 
  gt() %>% 
  cols_label(
    region_de_destino = md("**Región**"),
    establecimientos_abiertos = md("**Establecimientos**"),
    participacion  = md("**Part. %**")
  ) %>% 
  fmt_number(columns = 2, decimals = 0) %>% 
  fmt_percent(columns = 3, decimals = 1, sep_mark = ".", dec_mark = ",") %>% 
  cols_align(
    align = "center",
    columns = vars(establecimientos_abiertos, participacion
    ))  %>%
  opt_table_font(
    font = "Roboto"
  ) %>%
  tab_header(
    title = md("Establecimientos hoteleros afectados a la COVID-19"),
    subtitle = md(glue("**Cantidades y participación porcentual. {mes} {anio}**"))
    ) %>%
  tab_source_note(
    source_note = md(
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = vars(region_de_destino, establecimientos_abiertos, participacion),
      rows = region_de_destino == "Total")
    ) %>% 
...
#gtsave(tabla5_ficha, "salidas/tabla5.png")

#htmltools::save_html(html = tabla5_ficha, file = "salidas/tabla5.html")

#webshot::webshot("salidas/tabla5.html", file="salidas/tabla5.png", delay=3, vwidth = 900, vheight = 300)
```
<br>

+ Durante junio de 2021, 41 establecimientos permanecieron abiertos afectados a la COVID-19, prestando servicios no turísticos (cuarentenas, alojamiento de tripulaciones, etc), en las 49 localidades relevadas por la EOH: 12 en Litoral, 11 en CABA, 8 en Patagonia, 6 en Cuyo y 4 en Buenos Aires. En las regiones de Córdoba y Norte no se registró ningún establecimiento para dichos fines.
