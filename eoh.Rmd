...
description: "Resumen de los últimos datos de la Encuesta de Ocupación Hotelera (EOH), operativo conjunto entre el Instituto Nacional de Estadística y Censos (INDEC) y la Subsecretaría de Turismo, que releva información de los establecimientos hoteleros y parahoteleros de 51 destinos turísticos del país."
...
tabla1_ficha <- tablafinal_ficha %>%
  ...
  fmt_percent(columns = c(4,6,8), decimals = 1, sep_mark = ".", dec_mark = ",") %>% 
  cols_align(
    align = "center",
...
localidades_sin_datos <- c("x")#Localidades años anterior

localidades_insuficiencia <- c("Malargüe - Las Leñas","Pinamar","San Rafael")#Para localidades de este año
...
tabla4_A_ficha <- tabla4_A %>% arrange(region_de_destino) %>%
  ...
  fmt_percent(columns = c(4,6,8), sep_mark = ".", dec_mark = ",", decimals = 1) %>% 
  opt_table_font(
    font = "Roboto"
  ) %>%
