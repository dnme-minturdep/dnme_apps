...
tabla2 <- pernoctaciones_categoria %>% left_join(plazas_categoria) %>% 
  ...
  filter(year(indice_tiempo) == year(fecha)) %>%
  arrange(desc(categoria_del_hotel)) %>% #AD HOC AGOSTO 2021
  mutate(across(where(is.numeric), ~ case_when(is.infinite(.) ~ NA_real_,
                                           TRUE ~ .)))%>% #AD HOC AGOSTO 2021
...
tabla3 <- destinos %>% select(-observaciones) %>%
  ...
  mutate(pernoctes_total = pernoctes_residentes + pernoctes_no_residentes,
         ...
         estadia_total = round(pernoctes_total/viajeros_total, 1)) %>%  #AD HOC AGOSTO 2021
...
localidades_insuficiencia <- c("Formosa", "Calafate", "Puerto Iguazú", "Ushuaia")
localidades_sin_datos <- c("Gualeguaychú", "CABA", "Jujuy", "Las Grutas", "Rio Gallegos", "Viedma",
                           "La Angostura")
