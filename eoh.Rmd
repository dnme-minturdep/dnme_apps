...
tabla2 <- pernoctaciones_categoria %>% left_join(plazas_categoria) %>% 
  ...
  mutate(pernoctes = case_when(pernoctes == 0 ~ `is.na<-`(pernoctes),
      TRUE ~ pernoctes)) %>% 
  mutate(var_ia_pernoctes  = pernoctes / lag(pernoctes, n = 1) -1) %>%
  ungroup() %>%
...
tabla2_ficha <- tabla2 %>% 
  ...
  mutate(plazas_disponibles = plazas_disponibles/1000000,
         pernoctes_total = pernoctes_total/1000000,
         pernoctes_res = pernoctes_res/1000000,
         pernoctes_nores = pernoctes_nores/1000000) %>% 
  gt() %>% 
  cols_label(
...
grafico2 <- datos_grafico2 %>% 
  ...
  labs(title = "**Plazas disponibles, pernoctaciones y tasa de ocupación, según categoría**",
       subtitle = tolower(glue("**en millones y porcentaje, {mes} {anio}.** <span style='color:{cols_arg2[2]}'>**plazas disponibles**</span>**,** <span style='color:{cols_arg2[6]}'>**ocupadas**</span> **y** <span style='color: #3D4D7C'>**tasa de ocupación**</span>")),
                       x="",y="", fill = "",
       caption = "Fuente: Encuesta de Ocupación Hotelera (EOH), INDEC y Dirección Nacional de Mercados y Estadística, Subsecretaría de Desarrollo Estratégico, Ministerio de Turismo y Deportes")
