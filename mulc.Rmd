...
BP <- BP %>% mutate(year=substr(trim,1,4),fecha = case_when(substr(trim, 5,6) == "1T" ~ 
            ...
            summarise(year=year,fecha=fecha, emisivo_vyp_visitantes=emisivo_vyp_visitantes/1000000, receptivo_vyp_visitantes=receptivo_vyp_visitantes/1000000)
...
ing_acum_ia_2 <- filter(tabla1, year(fecha) == anio_datos-3 & month(fecha) <= month(last(tabla1$fecha))) %>% select(cobros) %>% sum()
...
#El valor de esta variable corresponde a  prepandemia (2019)
eg_acum_ia_2 <- filter(tabla1, year(fecha) == anio_datos-3 & month(fecha) <= month(last(tabla1$fecha))) %>% select(pagos) %>% sum()
...
+ En el acumulado enero - `r mes` de `r as.character(anio_datos)`, los egresos de divisas por turismo, por el mercado oficial de cambios, fueron de US\$`r eg_acum %>% round(1)` millones (+`r var_eg_acum_ia %>% round(1)`% vs mismo período de `r as.character(anio_datos-1)` y `r var_eg_acum_ia_2 %>% round(1)`% vs mismo período de `r as.character(anio_datos-3)`). Por su parte, los ingresos de divisas fueron de US\$`r ing_acum %>% round(1)` millones (`r var_ing_acum_ia %>% round(1)`% vs mismo período de `r as.character(anio_datos-1)` y `r var_ing_acum_ia_2 %>% round(1)`% vs mismo período de `r as.character(anio_datos-3)`). Esto significó una balanza deficitaria en US\$`r ((-1)*bza_acum) %>% round(1)` millones. 
...
g1 <- tabla1 %>% 
 ...
 geom_col(aes(fecha, balanza, fill = color_bza,text = paste0('fecha: ', format(fecha,"%b-%y"),
  ...
  scale_x_date(limits = c(min(as.Date(tabla1$fecha, origin = "1970-01-01"))-months(0),max(as.Date(tabla1$fecha, origin = "1970-01-01"))+months(1)),
...
g2 <- tabla2 %>% 
  ...
  scale_x_date(
    limits = c(min(tabla2$fecha)-months(0),max(tabla2$fecha)+months(1)),
               date_breaks = "2 months", 
               date_labels = "%b%y", 
               expand = c(0,0))+
  labs(title="",
...
tabla7_BP <- BP %>% filter(year>=2011) %>% select(fecha, receptivo_vyp_visitantes) 
                  
tabla7 <- right_join(tabla7_mulc, tabla7_BP, by = "fecha") %>% 
          rename("BP"="receptivo_vyp_visitantes") %>% 
...
tabla9_BP <- BP %>% filter(year>=2011) %>% select(fecha, emisivo_vyp_visitantes) 
                  
tabla9 <- right_join(tabla9_mulc, tabla9_BP, by = "fecha") %>% 
          rename("BP"="emisivo_vyp_visitantes") %>% 
