...
g1 <- tabla1 %>% 
 ...
 geom_col(data=tabla1_bis, aes(fecha, balanza, fill = color_bza,text = paste0('fecha: ', format(fecha,"%b-%y"),
  ...
  scale_x_date(limits = c(min(as.Date(tabla1$fecha, origin = "1970-01-01"))-months(2),max(as.Date(tabla1$fecha, origin = "1970-01-01"))+months(1)),
...
g2 <- tabla2 %>% 
  ...
  scale_x_date(
    limits = c(min(tabla2$fecha)-months(2),max(tabla2$fecha)+months(1)),
               date_breaks = "2 months", 
               date_labels = "%b%y", 
               expand = c(0,0))+
  labs(title="",
