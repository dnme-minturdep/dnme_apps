...
data_tabla_2 <- tabla_parques %>% 
  ...
  filter(anio >= 2022 & month == month(fecha)) %>% 
  pivot_wider(names_from  = c("anio", "residencia"), values_from = c(visitantes, acumulado)) %>% 
  mutate(across(where(is.numeric), ~ coalesce(.x, 0))) %>%
  clean_names() %>% 
  mutate(visitantes_2023_total = visitantes_2023_residentes + visitantes_2023_no_residentes,
         visitantes_2022_total = visitantes_2022_residentes + visitantes_2022_no_residentes,
  ...
  mutate(var_ia_visitantes_2023_tot = visitantes_2023_total/visitantes_2022_total-1,
         var_ia_visitantes_res = visitantes_2023_residentes/visitantes_2022_residentes-1,
         var_ia_visitantes_no_res = visitantes_2023_no_residentes/visitantes_2022_no_residentes-1,
         var_ia_acumulado_total  = acumulado_2023_total/ acumulado_2022_total-1) %>% 
  mutate(across(where(is.numeric), ~ coalesce(.x, 0))) %>%  
  arrange(-acumulado_2023_total) %>% 
  mutate(orden = rank(-acumulado_2023_total),
...
data_tabla_2 <- data_tabla_2 %>% 
  filter(presentacion != "Resto") %>% 
  bind_rows(resto_pn) %>% 
  mutate(participacion = round(acumulado_2023_total / sum(acumulado_2023_total), 3)) %>% 
  select(parque_nacional,
         visitantes_2023_total, var_ia_visitantes_2023_tot, 
         visitantes_2023_residentes, var_ia_visitantes_res,
         visitantes_2023_no_residentes, var_ia_visitantes_no_res,
         acumulado_2023_total, var_ia_acumulado_total, 
         participacion)
...
#tabla2 <- 
  ...
  cols_label(
    visitantes_2023_total         = md("Visitas"),
    var_ia_visitantes_2023_tot    = md("Var % i.a."),
    visitantes_2023_residentes    = md("Visitas") ,
    var_ia_visitantes_res         = md("Var % i.a."),
    visitantes_2023_no_residentes = md("Visitas"),
    var_ia_visitantes_no_res      = md("Var % i.a."), #SOLO EN ENERO
    acumulado_2023_total          = md("Visitas"), #SOLO EN ENERO
  ...
  cols_align(
    align = "center",
    columns = vars(visitantes_2023_total, var_ia_visitantes_2023_tot, visitantes_2023_residentes,
                   var_ia_visitantes_res, visitantes_2023_no_residentes, var_ia_visitantes_no_res,
                   acumulado_2023_total, var_ia_acumulado_total,  #SOLO ENERO
                   participacion)
  )  %>% 
  opt_table_font(
    font = list(
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = vars(var_ia_visitantes_2023_tot),
      rows = var_ia_visitantes_2023_tot < 0)
  ) %>%
  tab_style(
    style = list(
  ...
  tab_spanner(
    label = md("**TOTAL**"),
    columns = vars(visitantes_2023_total, 
                   var_ia_visitantes_2023_tot,
                   participacion #SOLO ENERO
                   )) %>%
  tab_spanner(
    label = md("**RESIDENTES**"),
    columns = vars(visitantes_2023_residentes,
                   var_ia_visitantes_res))  %>%
    tab_spanner(
    label = md("**NO RESIDENTES**"),
    columns = vars(visitantes_2023_no_residentes,
                   var_ia_visitantes_no_res))  %>%
  #tab_header(title = "ACUMULADO 2020")
  tab_spanner(
   label = md(glue("**ACUMULADO {anio}**")),
   columns = vars(acumulado_2023_total,
