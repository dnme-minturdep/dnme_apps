...
description: "Resumen de los últimos datos de visitas a Parques Nacionales según origen (condición de residencia) de los visitantes. La información se obtiene a partir de datos relevados por la Administración de Parques Nacionales."
...
data_tabla_2 <- tabla_parques %>% 
  ...
  filter(anio >= 2025 & month == month(fecha)) %>% 
  pivot_wider(names_from  = c("anio", "residencia"), values_from = c(visitantes, acumulado)) %>% 
  mutate(across(where(is.numeric), ~ coalesce(.x, 0))) %>%
  clean_names() %>% 
  mutate(visitantes_2026_total = visitantes_2026_residentes + visitantes_2026_no_residentes,
         visitantes_2025_total = visitantes_2025_residentes + visitantes_2025_no_residentes,
         # visitantes_2020_total = visitantes_2020_residentes + visitantes_2020_no_residentes,
         acumulado_2026_total = acumulado_2026_residentes + acumulado_2026_no_residentes,
         ...
         participacion = round(acumulado_2025_total / sum(acumulado_2025_total), 3)) %>%
  # mutate(across(where(is.numeric), ~ case_when(parque_nacional %in% parques ~ NA_real_,
  #                                          TRUE ~ .))) %>% 
  arrange(-participacion) %>%
  mutate(var_ia_visitantes_2026_tot = visitantes_2026_total/visitantes_2025_total-1,
         var_ia_visitantes_res = visitantes_2026_residentes/visitantes_2025_residentes-1,
         var_ia_visitantes_no_res = visitantes_2026_no_residentes/visitantes_2025_no_residentes-1,
         var_ia_acumulado_total  = acumulado_2026_total/ acumulado_2025_total-1) %>% 
  mutate(across(where(is.numeric), ~ coalesce(.x, 0))) %>%  
  arrange(-acumulado_2026_total) %>% 
  mutate(orden = rank(-acumulado_2026_total),
...
data_tabla_2 <- data_tabla_2 %>% 
  filter(presentacion != "Resto") %>% 
  bind_rows(resto_pn) %>% 
  mutate(participacion = round(acumulado_2026_total / sum(acumulado_2026_total), 3)) %>% 
  select(parque_nacional,
         visitantes_2026_total, var_ia_visitantes_2026_tot, 
         visitantes_2026_residentes, var_ia_visitantes_res,
         visitantes_2026_no_residentes, var_ia_visitantes_no_res,
         acumulado_2026_total, var_ia_acumulado_total, 
         participacion)
...
#tabla2 <- 
  ...
  cols_label(
    visitantes_2026_total         = md("Visitas"),
    var_ia_visitantes_2026_tot    = md("Var % i.a."),
    visitantes_2026_residentes    = md("Visitas") ,
    var_ia_visitantes_res         = md("Var % i.a."),
    visitantes_2026_no_residentes = md("Visitas"),
    var_ia_visitantes_no_res      = md("Var % i.a."), #SOLO EN ENERO
    acumulado_2026_total          = md("Visitas"), #SOLO EN ENERO
  ...
  cols_align(
    align = "center",
    columns = vars(visitantes_2026_total, var_ia_visitantes_2026_tot, visitantes_2026_residentes,
                   var_ia_visitantes_res, visitantes_2026_no_residentes, var_ia_visitantes_no_res,
                   acumulado_2026_total, var_ia_acumulado_total,  #SOLO ENERO
                   participacion)
  )  %>% 
  opt_table_font(
    font = list(
  ...
  tab_style(
    ...
    locations = cells_body(
      columns = vars(var_ia_visitantes_2026_tot),
      rows = var_ia_visitantes_2026_tot < 0)
  ) %>%
  tab_style(
    style = list(
  ...
  tab_header(title = toupper(glue("{mes} {anio}"))) %>% 
   ...
   columns = c(acumulado_2026_total,
                  var_ia_acumulado_total,
                  participacion)) %>%
    tab_options(column_labels.font.size = 14) %>% 
  tab_spanner(
    label = md("**TOTAL**"),
    columns = c(visitantes_2026_total, 
                   var_ia_visitantes_2026_tot #SOLO ENERO
                   )) %>%
  tab_spanner(
    label = md("**RESIDENTES**"),
    columns = c(visitantes_2026_residentes,
                   var_ia_visitantes_res))  %>%
    tab_spanner(
    label = md("**NO RESIDENTES**"),
    columns = c(visitantes_2026_no_residentes,
...
#   left_join(months)
# 
# # A PARTIR DEL 2025 SE ABRIO LA INFORMACIÓN POR CONDICION DE RESIDENCIA, SE LEVANTA LA INFORMACIÓN DE LA MISMA PIVOT EN OTRA SOLAPA
# 
...
# En el caso de los parques provinciales, durante el mes de `r mes` se registraron `r lbl_int(var_ia[var_ia$provincia == "Chubut", "Total"] %>% pull())` visitas a áreas naturales protegidas del Chubut, con `r lbl_int(var_ia[var_ia$provincia == "Chubut", "residentes"] %>% pull())` visitas de residentes y `r lbl_int(var_ia[var_ia$provincia == "Chubut", "no_residentes"] %>% pull())` de no residentes. En San Juan, el Parque Provincial Ischigualasto recibió un total de `r lbl_int(var_ia[var_ia$provincia == "San Juan (Ischigualasto)", "Total"] %>% pull())` visitas y el Gran Parque Iberá `r lbl_int(var_ia[var_ia$provincia == "Corrientes", "Total"] %>% pull())`.
 ...
 #    fmt_missing(columns = 1:10,
 #              missing_text = "-") %>% 
 #  tab_source_note(source_note = "Nota: a partir de enero 2025 se presenta información desagregada por residencia en el Parque Provincial Ishigualasto. A partir de septiembre 2025 se incluye información de visitas en el Área Natural Protegida Cabo Dos Bahías en Chubut y del Gran Parque Iberá en la provincia de Corrientes (incluye información de visitas por 8 portales de acceso al parque). ") %>% 
