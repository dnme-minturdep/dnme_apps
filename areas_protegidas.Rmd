...
```{r}
#Importo datos
parques_nacionales <- read_excel("/srv/DataDNMYE/areas_protegidas/areas_protegidas_nacionales/pivot_pn.xlsx",
                                 sheet = "base PN") %>% 
...
data_tabla_2 <- tabla_parques %>% 
  ...
  clean_names() %>% 
  mutate(visitantes_2022_total = visitantes_2022_residentes + visitantes_2022_no_residentes,
         visitantes_2021_total = visitantes_2021_residentes + visitantes_2021_no_residentes,
         visitantes_2020_total = visitantes_2020_residentes + visitantes_2020_no_residentes,
         acumulado_2022_total = acumulado_2022_residentes + acumulado_2022_no_residentes,
         ...
         participacion = round(acumulado_2022_total / sum(acumulado_2022_total), 3)) %>%
  # mutate(across(where(is.numeric), ~ case_when(parque_nacional %in% parques ~ NA_real_,
  #                                          TRUE ~ .))) %>% 
  arrange(-participacion) %>%
  mutate(var_ia_visitantes_2022_tot = visitantes_2022_total/visitantes_2021_total-1,
         var_ia_visitantes_res = visitantes_2022_residentes/visitantes_2021_residentes-1,
         var_ia_visitantes_no_res = visitantes_2022_no_residentes/visitantes_2021_no_residentes-1,
         var_ia_acumulado_total  = acumulado_2022_total/ acumulado_2021_total-1) %>% 
  mutate(across(where(is.numeric), ~ coalesce(.x, 0))) %>%  
  arrange(-acumulado_2022_total) %>% 
  mutate(orden = rank(-acumulado_2022_total),
...
data_tabla_2 <- data_tabla_2 %>% 
  filter(presentacion != "Resto") %>% 
  bind_rows(resto_pn) %>% 
  mutate(participacion = round(acumulado_2022_total / sum(acumulado_2022_total), 3)) %>% 
  select(parque_nacional,
         visitantes_2022_total, var_ia_visitantes_2022_tot, 
         visitantes_2022_residentes, var_ia_visitantes_res,
         visitantes_2022_no_residentes, var_ia_visitantes_no_res,
         acumulado_2022_total, var_ia_acumulado_total, 
         participacion)
...
#tabla2 <- 
  ...
  cols_label(
    visitantes_2022_total         = md("Visitas"),
    var_ia_visitantes_2022_tot    = md("Var % i.a."),
    visitantes_2022_residentes    = md("Visitas") ,
    var_ia_visitantes_res         = md("Var % i.a."),
    visitantes_2022_no_residentes = md("Visitas"),
    var_ia_visitantes_no_res      = md("Var % i.a."), #SOLO EN ENERO
    acumulado_2022_total          = md("Visitas"), #SOLO EN ENERO
  ...
  cols_align(
    align = "center",
    columns = vars(visitantes_2022_total, var_ia_visitantes_2022_tot, visitantes_2022_residentes,
                   var_ia_visitantes_res, visitantes_2022_no_residentes, var_ia_visitantes_no_res,
                   acumulado_2022_total, var_ia_acumulado_total,  #SOLO ENERO
                   participacion)
  )  %>% 
  opt_table_font(
    font = list(
  ...
  tab_source_note(
    source_note = md(
      "**Fuente**: Dirección Nacional de Mercados y Estadística, Subsecretaría de Desarrollo Estratégico, Ministerio de Turismo y Deportes en base a la informacion suministrada por la Dirección de Mercadeo de la Dirección Nacional de Uso Público, APN")
  ) %>%
  #   tab_source_note(
  #   source_note = md(
  #     "**Nota**: En enero 2022 se incorporó información de visitas del Parque Patagonia en la provincia de Santa Cruz")
  # ) %>%
  tab_style(
    style = list(
    ...
    locations = cells_body(
      columns = vars(var_ia_visitantes_2022_tot),
      rows = var_ia_visitantes_2022_tot < 0)
  ) %>%
  tab_style(
    style = list(
  ...
  tab_spanner(
    label = md("**TOTAL**"),
    columns = vars(visitantes_2022_total, 
                   var_ia_visitantes_2022_tot,
                   participacion #SOLO ENERO
                   )) %>%
  tab_spanner(
    label = md("**RESIDENTES**"),
    columns = vars(visitantes_2022_residentes,
                   var_ia_visitantes_res))  %>%
    tab_spanner(
    label = md("**NO RESIDENTES**"),
    columns = vars(visitantes_2022_no_residentes,
                   var_ia_visitantes_no_res))  %>%
  #tab_header(title = "ACUMULADO 2020")
  tab_spanner(
   label = md(glue("**ACUMULADO {anio}**")),
   columns = vars(acumulado_2022_total,
...
chubut <-  read_csv("/srv/DataDNMYE/areas_protegidas/areas_protegidas_provinciales/base_trabajo/bases_chubut.csv") %>% 
  filter(Mes != "total") %>% 
...
sanjuan <-readxl::read_excel("/srv/DataDNMYE/areas_protegidas/areas_protegidas_provinciales/Base_parq_prov_ischigualasto.xlsx") %>% 
...
tabla_parques <- bind_rows(chubut, sanjuan) %>% 
...
```{r}
 var_ia %>% 
  ...
  gt_theme_dnmye() %>% 
    fmt_missing(columns = 1:10,
              missing_text = "-") %>% 
  tab_source_note(source_note = "-: no se cuenta con información desagregada por residencia para San Juan.") %>% 
