...
int_pasajeros_acumulados_total <- pasajeros_mes %>% 
  select(acum_anual) %>% 
  pull %>%
  format(., big.mark = ".")
...
Las provincias de **`r primer_rank_prov`, `r segun_rank_prov` y Córdoba** son aquellas que **más vuelos de cabotaje recibieron** durante el último mes.
...
herfintal_prov_empre_2022 <- cabotaje %>% 
...
herfintal_prov_empre %>%
  ...
  complete(anio_local,
           ...
           fill = list(frecuencias_x_destino_aerolinea = 0,
                       ...
                       federalizacion = 0)) %>% 
  left_join(herfintal_prov_empre_2022) %>% 
  ggplot() +
...
freq_ciudad_dest_23 <- freq_ciudad_dest_23 %>% 
  ...
  mutate(variacion = (frecuencias_23-frecuencias_22)/frecuencias_22,
...
freq_ciudad_dest_23 <- freq_ciudad_dest_23 %>% 
  ...
  mutate(across(.cols = contains("frecuencias"), ~ round(., 0)),
         posicion_22 = if_else(is.na(posicion_22),
                               "-",
                               posicion_22))
...
freq_ciudad_dest_23[sapply(freq_ciudad_dest_23, is.infinite)] <- NA
...
vuelos_int <- vuelos_int_23 %>% 
  full_join(vuelos_int_22, by = "destino_localidad_etiqueta") %>% 
  filter(!is.na(posicion_23)) %>% 
  mutate(variacion = (frecuencias_23-frecuencias_22)/frecuencias_22,
         destino_localidad_etiqueta = case_when(posicion_23 >= 20 ~ "Resto",
                                                T ~ destino_localidad_etiqueta)) %>%
  ...
  summarise(frecuencias_23 = sum(frecuencias_23, na.rm = T),
            ...
            posicion_22 = first(posicion_22)) %>% 
  mutate(variacion = (frecuencias_23-frecuencias_22)/frecuencias_22,
         participacion_23 = frecuencias_23/sum(frecuencias_23),
         posicion_23 = case_when(destino_localidad_etiqueta == "Resto" ~ "-",
                                 T ~ as.character(posicion_23)),
         posicion_22 = case_when(destino_localidad_etiqueta == "Resto" ~ "-",
                                 T ~ as.character(posicion_22))) %>%
...
aerolineas_int <- aerolineas_int_23 %>% 
  full_join(aerolineas_int_22, by = "empresa_agrup_def") %>% 
  filter(!is.na(posicion_23)) %>% 
  mutate(variacion = (frecuencias_23 - frecuencias_22)/frecuencias_22,
         empresa_agrup_def = case_when(posicion_23 >= 20 ~ "Otras empresas",
                                       T ~ empresa_agrup_def)) %>%
  group_by(empresa_agrup_def) %>% 
  summarise(frecuencias_23 = sum(frecuencias_23, na.rm = T),
            frecuencias_22 = sum(frecuencias_22, na.rm = T),
...
tabla_grande <- tabla %>% 
  ...
  opt_table_font(
    font = list(
      google_font(name = "Encode Sans"))) %>% 
  fmt_percent(columns = c(9:12), decimals = 1, dec_mark = ",", sep_mark = ".") %>% 
  fmt_missing(
    columns = everything(),
    rows = everything(),
    missing_text = "-") %>%
...
tabla %>% 
  select(-ends_with("_previo")) %>% 
  select(-c(anio_2017, anio_2018, anio_2020, anio_2021)) %>% 
  gt() %>% 
  tab_header(
    title = md("__PASAJEROS TOTALES EN VUELOS DE CABOTAJE POR AEROPUERTO__")) %>%  
  tab_source_note(
