...
<!-- En este sentido, por ejemplo, en el mes de **`r datos_mes_anio`**, **el mercado de cabotaje** nacional mostró **`r var_2019_vuelos` vuelos, un `r var_2019_pasajeros` de pasajeros y un `r var_2019_asientos` de asientos en comparación al mismo mes de 2019**[^1]. -->

<!-- [^1]: Por su parte, el **mercado internacional** resgistró un **`r int_var_2019_vuelos` de vuelos, un `r int_var_2019_pasajeros` de pasajeros y un `r int_var_2019_asientos` de asientos en respecto al mismo período**. -->
...
En total, en el acumulado anual, se realizaron `r vuelos_acumulados_total` vuelos, un `r variacion_interanual_acum` si se compara con el mismo período del año previo.
...
<!-- SACAR EN ENERO, PONER EN EL RESTO DE LOS MESES -->
En el acumulado anual, ello significa que viajaron hasta el momento `r pasajeros_acumulados_total` personas, un `r variacion_interanual_acum_pasajeros` en comparación al mismo intervalo del año anterior.
...
Con respecto al número de **asientos** disponibles en vuelos internacionales, **`r int_asientos_mes_total` plazas** fueron contadas, lo que equivale a un `r int_variacion_interanual_asientos` frente a `r datos_mes_anio` del año anterior. En el acumulado anual, `r int_asientos_acumulados_total` asientos fueron sumados, lo que supone una variación de `r int_variacion_interanual_acum_asientos`
<!-- SUMAR SI NO ES ENERO -->
...
#   ungroup() %>% 
#   filter(anio_local == ifelse(last(mes_local == 1),max(anio_local)-1,max(anio_local))) %>% 
#   filter(mes_local == ifelse(max(mes_local) == 12,max(mes_local),max(mes_local) - 1)) %>% 
...
tabla <- pasajeros_totales_cabotaje %>% 
  ...
  janitor::adorn_totals(where = "row",
                        na.rm = T) %>% 
  mutate(var_acum = (anio_2025/anio_2024)-1) 

tabla %>% 
  arrange(desc(anio_2025)) %>% 
  gt() %>% 
  tab_header(
    title = md("__PASAJEROS TOTALES EN VUELOS DE CABOTAJE POR LOCALIDAD__"),
