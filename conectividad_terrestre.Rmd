...
description: "Resumen de datos de flujo terrestre, en base a los registros de servicios regulares interurbanos de buses de media y larga distancia de la Comisión Nacional de Regulación del Transporte (CNRT)."
output:
...
# Base Actual
base_total <- open_dataset("/srv/DataDNMYE/cnrt/base_de_trabajo/base_final/cnrt_total_historica_2026-01-01_a_2026-05-31.parquet") # Actualizar

# Base Año Previo
base_vieja <- open_dataset("/srv/DataDNMYE/cnrt/base_de_trabajo/base_final/cnrt_total_historica_2025-01-01_a_2025-12-31.parquet")
...
[^1]: Un servicio de turismo es aquel que se efectúa a fin de atender un servicio de transporte integrado en una programación turística, como complemento a una actividad de tal naturaleza, trasladando a un contingente y expresamente identificando las personas que lo componen, conforme a un contrato celebrado a tal efecto.

```{r}
base_gt %>% 
  #filter(periodo != "Acumulado") %>%  #poner en enero
...
freq_ciudad_dest_24 %>%
  ...
  cols_label(
    ...
    situacion = md(""),
    posicion_24 = md("**2026**"),
    pasajeros_23 = md("**2025**"),
    pasajeros_24 = md("**2026**"),
    variacion = md("**v.a. %**"),
    participacion_24 = md("**Participación 2026 (%)**")) %>% 
  cols_align(
    align = "center",
    columns = everything()) %>% 
  opt_table_font(
