...
data_grafico <- turistas_vias %>% filter(via == "TOTAL") %>% mutate(period = ymd(as.character(glue::glue("{anio}/{mes}/01"))),
  ...
  filter(period <= fecha & period >= "2018-04-01" ) %>% #DEFINO PERÍODO DE INICIO
...
grafico_1 <- ggplot(data_grafico)  +
  ...
  geom_point(aes(period, emisivo, text = paste('fecha:', format(fecha,"%b-%y"),
  ...
  geom_point(aes(period, receptivo, text = paste('fecha:', format(fecha,"%b-%y"),
                                              '<br>turistas:', format(round(receptivo,0),big.mark="."), 
                                              '<br>turismo:',"Receptivo")),
             size = 1.1, color = cols_arg) +
  geom_col(aes(period, balanza, fill = balanza_color,text = paste('fecha:', format(fecha,"%b-%y"),
