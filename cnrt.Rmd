...
asientos_mes <- base_total %>%
  distinct(id_servicio, .keep_all = T) %>% 
  group_by(anio, mes) %>%
...
En el mes de **`r datos_mes_anio`**, fueron contabilizados **`r viajes_mes_total` viajes** de buses de media y larga distancia en todo el país, lo que representa un **`r variacion_interanual` frente a un año atrás**[^2]. En total, en el acumulado anual, se realizaron `r viajes_acumulados_total` servicios regulares, un `r variacion_interanual_acum` si se compara con el mismo período del año previo.

[^2]: Para ver la serie temporal completa de viajes, asientos y pasajeros totales, acceder al [tablero de conectividad terrestre](https://tableros.yvera.tur.ar/conectividad_terrestre/).
...
En lo respectivo a las regiones turísticas, **el `r region_1` fue la que más pasajeros recibió** durante el mes de `r datos_mes_anio` (**`r region_1_total`** en total). A la misma, le siguieron las regiones `r region_2` y `r region_3`, con `r region_2_total` y `r region_3_total` pasajeros, respectivamente.
...
```{r}
cnrt <- arrow::open_dataset("/srv/DataDNMYE/cnrt/base_de_trabajo/base_final/")
...
Las provincias de **`r primer_rank_prov`, `r segun_rank_prov` y `r tercer_rank_prov`** son aquellas a las que más pasajeros arribaron durante el último mes en buses de media y larga distancia. Las mismas contabilizaron `r primer_rank_prov_tot`, `r segun_rank_prov_tot` y `r tercer_rank_prov_tot`, respectivamente.
...
Por su parte, a nivel urbano, **`r primer_rank_loc`, `r segun_rank_loc` y `r tercer_rank_loc`** son aquellas localidades del país que más pasajeros recibieron en el período en cuestión. En total, `r primer_rank_loc_tot`, `r segun_rank_loc_tot` y `r tercer_rank_loc_tot` personas arribaron a dichos destinos.
...
#   tab_source_note(
#     source_note = md("**Fuente:** MINTURDEP en base a información de CNRT.")) %>% 
#   cols_label(
...
A **`r datos_mes_anio`**, existían en el país **`r rutas_numero_total` rutas con 4 o más frecuencias mensuales**, `r rutas_numero_cabotaje_diferencia` con respecto al mismo mes del año pasado (`r rutas_numero_total_previo`). De dicho total, `r rutas_numero_federales` eran federales y `r rutas_numero_portenias` eran porteñas.
...
graf_4 <- 
  ...
  labs(title = "Cantidad de rutas",
       ...
       x = "",
       color = "",
       caption = glue::glue("Fuente: MINTURDEP en base a información de CNRT\n Nota: En rojo, los meses de {datos_mes_anio}.")) +
  theme_minimal() +
  theme(legend.position = "bottom",
        axis.text.x = element_text(angle = 45,
                                   hjust = 1,
