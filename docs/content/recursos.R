...
data.frame(
  recurso = c("Turismo Internacional", "Ocupación Hotelera", "Naturaleza", "EVYTH", "Explorador", "PUNA", "MULC"), 
  src = c("img/ti.png", "img/eoh.png","img/parques.png","img/evyth.png","img/ti_dash.png" , "img/puna.png", "img/mulc.png"), 
  ...
  tags = c("Reporte","Reporte","Reporte","Reporte","Tablero", "Tablero", "Reporte"),
  Todos = c(1, 1, 1, 1, 1, 1, 1), 
  Tablero = c(0, 0, 0, 0, 1, 1, 0), 
  Reporte = c(1, 1, 1, 1, 0, 0, 1)) %>% 
  as_tibble() %>% 
  mutate(orden = c(1, 5, 4, 3, 2, 6, 7)) %>%   # OREDNO POR TEMA Y NO POR TIPO DE RECURSO 
  arrange(orden)  %>% 
