...
### (Publicado en `r format(Sys.Date(),'%B')` `r format(Sys.Date(),'%Y')` con datos actualizados a noviembre 2022)
...
grafico_empleo_series <- ggplot(indice.empleo,aes(x=as.Date(fecha),y=indice,color=rct,group=rct,
  ...
  scale_x_date(breaks = seq.Date(as.Date(min(indice.empleo$fecha))+months(4),as.Date(max(indice.empleo$fecha)),by="6 months"),
...
grafico_empleo_series_2 <- ggplot(data_grafico_empleo_todas)+
 ...
 scale_x_date(breaks = seq.Date(as.Date(min(indice.empleo$fecha))+months(10),as.Date(max(indice.empleo$fecha)),by="12 months"),
               #date_breaks = "6 months",
               date_labels = "%b%y",
               expand=c(0,0))+
  scale_color_manual(values=cols_arg2)+
...
grafico_empresas_series <- ggplot(indice.empresas,aes(x=as.Date(fecha),y=indice,color=rct,group=rct,
  ...
  #geom_point()+
  scale_y_continuous(labels=scales::number_format(decimal.mark = ",",scale = 100,accuracy = 2))+
 scale_x_date(breaks = seq.Date(as.Date(min(indice.empleo$fecha)+months(3)),as.Date(max(indice.empleo$fecha)),by="6 months"),
               #date_breaks = "6 months",
               date_labels = "%b%y",
               expand=c(0,0))+
  # ggrepel::geom_label_repel(data=filter(indice.empleo,fecha==max(indice.empleo$fecha)),
...
grafico_empresas_series_2 <- ggplot(data_grafico_empresas_todas)+
  ...
  scale_x_date(breaks = seq.Date(as.Date(min(indice.empleo$fecha)+months(9)),as.Date(max(indice.empleo$fecha)),by="12 months"),
               #date_breaks = "6 months",
               date_labels = "%b%y",
               expand=c(0,0))+
  scale_color_manual(values=cols_arg2)+
