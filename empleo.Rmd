...
### (Publicado en `r format(Sys.Date(),'%B')` `r format(Sys.Date(),'%Y')` con datos actualizados a febrero 2022)
...
grafico_empleo_cat_rct <- ggplot(data_grafico_empleo_cat_rct,aes(x=reorder(cat.rct,puestos),y=puestos))+
  ...
  labs(x="",y="",title= "Empleo en Ramas Características del Turismo",
       subtitle = glue("Miles de puestos. {format(max(as.Date(puestos$fecha)),'%B')} {format(max(as.Date(puestos$fecha)),'%Y')} <span style='color:#f7a1c9'>Variación i.a.</span> y <span style='color:#F7941E'>Var. mismo período 2020</span>"), caption="**Fuente:** DNMyE en base a MDP")
...
ramas %>% 
  ...
  datatable(caption=glue("Ramas seleccionadas y valores de { format(as.Date(max(puestos$fecha)),'%B %Y')} (puestos) y {format(as.Date(fecha_empresas),'%B %Y')} (% de mujeres y empresas)"))
```
