...
description: "Puestos de trabajo registrados, empleo por género y empresas en ramas turísticas a partir de datos abiertos publicados por el Ministerio de Desarrollo Productivo en base al SIPA"
...
### (Publicado en `r format(Sys.Date(),'%B')` `r format(Sys.Date(),'%Y')` con datos actualizados a diciembre 2021)

```{r}
#INSUMOS
empleo <- read.csv("https://cdn.produccion.gob.ar/cdn-cep/datos-por-actividad/puestos/puestos_total_empresas_mensual_por_clae6.csv")
...
empleadores <- read.csv("https://cdn.produccion.gob.ar/cdn-cep/datos-por-actividad/empleadoras/empleadoras_total_empresas_por_clae6.csv")

#ATENCIÓN: EN ESTA OPORTUNIDAD LOS DATOS DE EMPRESAS LLEGAN HASTA noviembre 2021, NO AGOSTO
fecha_empresas <- max(empleadores$fecha)

puestos.genero <- read.csv("https://cdn.produccion.gob.ar/cdn-cep/datos-por-actividad/share-mujer/share_mujer_total_empresas_mensual_por_clae6.csv")
################################################################################
...
<br>
A continuación se presenta información sobre empleo registrado en Ramas Características del Turismo, con datos abiertos publicados por el [Ministerio de Desarrollo Productivo (MDP)](https://datos.produccion.gob.ar/), al mes de `r format(as.Date(max(puestos$fecha)),"%B %Y")`.

La metodología de las series elaboradas por el MDP se encuentra [aquí](https://datos.produccion.gob.ar/dataset/15211f62-04dc-42ed-acdd-ff2bbcaf4779/resource/0ee975c9-8241-4d29-9013-7e190644faad/download/metodologia-da.pdf).
...
En la comparación con `r format(as.Date(max(puestos$fecha))-years(2),"%B %Y")`, la variación en ramas turísticas fue de `r format(round(100*indice.empleo[indice.empleo$fecha == max(indice.empleo$fecha) & indice.empleo$rct == "RCT","var.a.2.anios"],1),decimal.mark=",") `%, mientras que en el resto de las ramas fue de `r format(round(100*indice.empleo[indice.empleo$fecha == max(indice.empleo$fecha) & indice.empleo$rct == "NO RCT","var.a.2.anios"],1),decimal.mark=",") `%
...
grafico_empleo_cat_rct <- ggplot(data_grafico_empleo_cat_rct,aes(x=reorder(cat.rct,puestos),y=puestos))+
  ...
  geom_label(aes(label=paste0(format(round(100*var.i.a.,1),decimal.mark = ","),"%"),y=max(puestos)*1.25),
             color="#f7a1c9")+
  geom_label(aes(label=paste0(format(round(100*var.a.2.anios,1),decimal.mark = ","),"%"),y=max(puestos)*1.45),
             color = "#F7941E")+
  ...
  labs(x="",y="",title= "Empleo en Ramas Características del Turismo",
       subtitle = glue("Miles de puestos. {format(max(as.Date(puestos$fecha)),'%B')} {format(max(as.Date(puestos$fecha)),'%Y')} <span style='color:#f7a1c9'>Variación i.a.</span> y <span style='color:#F7941E'>Var. mismo período 2019</span>"), caption="**Fuente:** DNMyE en base a MDP")
...
En la comparación con `r format(as.Date(max(indice.empresas$fecha))-years(2),"%B %Y")`, la variación en ramas turísticas fue de `r format(round(100*indice.empresas[indice.empresas$fecha == max(indice.empresas$fecha) & indice.empresas$rct == "RCT","var.a.2.anios"],1),decimal.mark=",") `%, mientras que en el resto de las ramas fue de `r format(round(100*indice.empresas[indice.empresas$fecha == max(indice.empresas$fecha) & indice.empresas$rct == "NO RCT","var.a.2.anios"],1),decimal.mark=",") `%
...
grafico_empresas_cat_rct <- ggplot(data_grafico_empresas_cat_rct,aes(x=reorder(cat.rct,empresas),y=empresas))+
  ...
  geom_label(aes(label=paste0(format(round(100*var.i.a.,1),decimal.mark = ","),"%"),y=max(empresas)*1.25),
             color="#f7a1c9")+
  geom_label(aes(label=paste0(format(round(100*var.a.2.anios,1),decimal.mark = ","),"%"),y=max(empresas)*1.45),
             color = "#F7941E")+
  geom_hline(yintercept = 0, size = 0.1) +
  #scale_fill_manual(values = cols_arg2[2])+
  scale_y_continuous(labels = scales::number_format(big.mark = ".", decimal.mark = ",")) +
  ...
  labs(x="",y="",title= "Empleadores en Ramas Características del Turismo",
       subtitle = glue("Miles de empresas. {format(as.Date(fecha_empresas),'%B')} {format(as.Date(fecha_empresas),'%Y')} <span style='color:#f7a1c9'>Variación i.a.</span> y <span style='color:#F7941E'>Var. mismo período 2019</span>"), caption="**Fuente:** DNMyE en base a MDP")
...
ramas %>% 
  ...
  select(clae6,"Descripción" = clae6_desc,"Categoría" = cat.rct,"Puestos"=puestos,
         "% de mujeres" = share_mujer) %>% 
  left_join(puestos %>% filter(!is.na(empresas)) %>% filter(fecha == max(fecha)) %>% select(clae6,empresas),by="clae6") %>% 
  rename("CLAE" = clae6,"Empresas"=empresas) %>% 
  datatable(caption=glue("Ramas seleccionadas y valores de{ format(as.Date(max(puestos$fecha)),'%B %Y')} (exepto empresas -{format(as.Date(fecha_empresas),'%B %Y')}-)"))
```
